        ...
        <form @submit.prevent="handleSubmit" class="max-w-3xl mx-auto space-y-6" v-if="isAdmin">
            ...
            <div class="pt-5">
                <div class="flex justify-end space-x-3">
                    <router-link to="/notice" class="px-4 py-2 rounded-md transition-colors duration-200 text-sm font-medium bg-gray-200 text-gray-700 hover:bg-gray-300"> 취소 </router-link>
                    <button type="submit" class="px-4 py-2 rounded-md transition-colors duration-200 text-sm font-medium btn-base bg-[var(--dream-main)] hover:bg-opacity-80 text-white">
                        {{ isEditMode ? "수정 완료" : "등록" }}
                    </button>
                </div>
            </div>
